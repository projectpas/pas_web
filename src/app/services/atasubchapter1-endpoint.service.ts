...
export class ATASubChapter1Endpoint extends EndpointFactory {
    ...
    getATASubChapter1Endpoint<T>(masterCompanyId?): Observable<T> {
        let endpointUrl = `${this.actionsUrl}/${masterCompanyId !==undefined ? masterCompanyId : 1 }`;
        return this.http.get<T>(endpointUrl, this.getRequestHeaders())
            .catch(error => {
                return this.handleErrorCommon(error, () => this.getATASubChapter1Endpoint(masterCompanyId));
        });
    }
