...
import { editValueAssignByCondition, getObjectByValue } from '../../../generic/autocomplete';
    ...
    if (this.allShipViaInfo && this.allShipViaInfo.length != 0) {
      for (var i = 0; i < this.allShipViaInfo.length; i++) {
        if (this.allShipViaInfo[i].shippingViaId == this.ShipViaEditID) {
          ...
          this.addShipViaFormForFreight.Description = this.allShipViaInfo[i].description;
        }
      }
    }
  }
