...
export class SubWorkOrderComponent implements OnInit {
    ...
    ngOnInit() {
        ...
        // console.log("parems",queryParamsData)
...
//  console.log("part numbers",this.subWorkOrderPartNumbers);
        }, error => {
...
subWorkOrder.map((x, index) => {
    ...
    // x.mappingItemMasterId= getObjectById('mappingItemMasterId', x.mappingItemMasterId, x.revisedParts),
...
// console.log("subworkorder details",this.subWorkOrderPartNumbers)
        this.workOrderService.createSubWorkOrderGrid(subWorkOrder).subscribe(res=>{
// this.changeofMPN(res[0]);
...
this.alertService.showMessage(
        ...
        if (this.workOrderId && this.mpnId) {
            this.workOrderService.getSubWorkOrderHeaderByWorkOrderId(this.workOrderId, this.mpnId).subscribe(res => {
                ...
                this.workOrderDetails = {
                    ...
                    // customerName:res.custName,
                    // customerCode:res.customerCode ? res.customerCode :res.custName,
    ...
    saveSubWorkOrder() {
        ...
        if (!this.isEdit) {
            ...
            // updateSubWorkOrderHeaderBySubWorkOrderId
            this.workOrderService.createSubWorkOrderHeaderByWorkOrderId(data).subscribe(res => {
                this.isEdit = true;
                this.showTabsGrid = true;
                this.showGridMenu = true;
    ...
    selectedCondition(value,currentRecord, index,) {
     ...
     selectedStage(value,currentRecord, index,) {
        ...
        calculatePartTat(workOrderPart) { 
            const data=this.subWorkOrderGeneralInformation
