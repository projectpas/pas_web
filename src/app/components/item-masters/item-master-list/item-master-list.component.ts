...
export class ItemMasterListComponent implements OnInit, AfterViewInit, AfterContentChecked {
  ...
  permissionAddCheck = [
    ModuleConstants.Item + "." + PermissionConstants.Add,
  ...
  permissionUpdateCheck = [
    ModuleConstants.Item + "." + PermissionConstants.Update,
  ...
  constructor(
    ...
    this.isActiveButton = this.authService.checkPermission([
      ModuleConstants.Item + "." + PermissionConstants.Update,
    ]);
    this.isDownload = this.authService.checkPermission([
      ModuleConstants.ItemList + "." + PermissionConstants.Download,
    ]);
    this.isDelete = this.authService.checkPermission([
      ModuleConstants.Item + "." + PermissionConstants.Delete,
    ]);

    this.isATAView = this.authService.checkPermission([
      ModuleConstants.Item_ATAChapter + "." + PermissionConstants.View,
    ]);
    this.isAircraftView = this.authService.checkPermission([
      ModuleConstants.Item_AircraftInformation + "." + PermissionConstants.View,
    ]);
    this.isNHA = this.authService.checkPermission([
      ModuleConstants.Item_NHATLA + "." + PermissionConstants.View,
    ]);
    this.isExchange = this.authService.checkPermission([
      ModuleConstants.Item_ExchangeLoan + "." + PermissionConstants.View,
    ]);

    this.isPurchase = this.authService.checkPermission([
      ModuleConstants.Item_PurchaseSales + "." + PermissionConstants.View,
    ]);
  }
  ...
  ngOnInit(): void {
    ...
    this.ptnumberlistdata();
		...
		this.cols = [
			...
			{ field: 'stockType', header: 'Stk Type',width:"90px" },
  ...
  openEdits(row) {
    ...
    this.itemMasterService
      .getCapabilityData(row.itemMasterId)
      .subscribe((data) => {
    ...
    this.router.navigateByUrl(
      `/itemmastersmodule/itemmasterpages/app-item-master-non-stock/edit/${row.itemMasterId}`
    );
  }
  ...
  openEdited(row) {
    ...
    this.activeIndex = 2;
    this.router.navigateByUrl(
      "/itemmastersmodule/itemmasterpages/app-item-master-equipment"
    );
  }
  ...
  private onitemmasterSuccessful(allWorkFlows, value) {
    for (let i = 0; i < allWorkFlows.length; i++) {
      if (allWorkFlows[i]["manufacturer"]) {
        allWorkFlows[i]["manufacturerdesc"] =
          allWorkFlows[i]["manufacturer"]["name"];
      }

      if (allWorkFlows[i]["itemClassification"]) {
        allWorkFlows[i]["classificationdesc"] =
          allWorkFlows[i]["itemClassification"]["description"];
      }
    }
    this.stockTable = true;
    ...
    if (value == "Stock") {
      ...
      this.cols2 = [
        ...
        {
          field: "equipmentValidationDescription",
          header: "Equipment Validation Type",
        },
        { field: "manufacturerdesc", header: "Manufacturer" },
      ];
    ...
    if (this.allRolesInfo.length > 0) {
      for (let i = 0; i < this.allRolesInfo.length; i++) {
        if ((this.allRolesInfo[i].entityName = "ItemMasterMain")) {
          ...
          if (
            (this.allRolesInfo[i].permittedEditActionDescription = "Delete")
          ) {
            this.Delete = true;
  ...
  getNtaeData(itemMasterId) {
    ...
    this.itemMasterService
      .getnhatlaaltequpartlis(reqDatas1)
      .subscribe((res) => {
        this.ntaeData3 = res;
    ...
    this.itemMasterService
      .getequivalencypartlist(reqDatas2)
      .subscribe((res) => {
        this.ntaeData4 = res;
  ...
  getATAMappedDataByItemMasterId(itemMasterId) {
    this.itemMasterService
      .getMappedATADetails(itemMasterId)
      .subscribe((res) => {
        this.ataMappedList = res.map((x) => {
          return {
            ...
            ataSubChapterDescription:
              x.ataSubChapterCode + " - " + x.ataSubChapterDescription,
          };
        });
      });
  }
  ...
  getAircraftMappedDataByItemMasterId(itemMasterId) {
    this.itemMasterService
      .getMappedAirCraftDetails(itemMasterId)
      .subscribe((data) => {
  ...
  getItemMasterById(itemMasterId) {
    this.itemMasterService
      .getItemMasterDetailById(itemMasterId)
      .subscribe((res) => {
  ...
  public navigateTogeneralInfononstock() {
    ...
    this.itemMasterService.enableExternal = false;
    this.route.navigateByUrl(
      "/itemmastersmodule/itemmasterpages/app-item-master-non-stock"
    );
  }
  ...
  toGetAllDocumentsList(itemMasterId) {
    var moduleId = 22;
    this.commonService
      .GetDocumentsList(itemMasterId, moduleId)
      .subscribe((res) => {
  ...
  getAircraftAuditHistory(rowData) {
    this.itemMasterService
      .getItemMasterAircraftAuditHistory(rowData.itemMasterAircraftMappingId)
      .subscribe((res) => {
  ...
  openHistoryDoc(rowData) {
    this.commonService
      .GetAttachmentAudit(rowData.attachmentDetailId)
      .subscribe((res) => {
