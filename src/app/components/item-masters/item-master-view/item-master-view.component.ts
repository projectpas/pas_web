...
export class ItemMasterViewComponent implements OnInit, AfterViewInit {
  ...
  isNHA: boolean = true;
  isExchangLoan: boolean = true;
  isExpoinfo: boolean = true;
  ...
  constructor(
    ...
    this.isCaps = this.authService.checkPermission([
    ...
    this.isExchangLoan = this.authService.checkPermission([
      ModuleConstants.Item_ExchangeLoan + "." + PermissionConstants.View,
    ]);
    this.isExpoinfo = this.authService.checkPermission([
      ModuleConstants.Item_ExportInformation + "." + PermissionConstants.View,
    ]);
  }
  ngOnInit(): void {
    if (this.isView) {
  ...
  handleChangeforstock(rowData, e) {
    if (e.checked == false) {
      ...
      this.itemMasterService
        .updateActionforActiveforstock(this.sourceAction)
        .subscribe(
  ...
  getItemMasterById() {
    ...
    this.itemMasterService
      .getItemMasterDetailById(this.itemMasterRowData.itemMasterId)
      .subscribe(
        (res) => {
          this.viewItemMaster = res[0];
          this.viewItemMaster.integrationPortal = this.viewItemMaster.integrationPortal.replace(
            /,/g,
            ", "
          );
  ...
  openHistoryOfPurchaseAndSales(content, rowData) {
    ...
    this.commonService
      .GetPurchaseAndSalesAuditHistory(rowData.itemMasterPurchaseSaleId)
      .subscribe(
  ...
  getAtaChapterAuditHistory(rowData) {
    this.itemMasterService
      .getATAMappedAudit(rowData.itemMasterATAMappingId)
      .subscribe((res) => {
  ...
  getAircraftMappedDeletdDataByItemMasterId(event) {
    ...
    this.itemMasterService
      .getDeletdMappedAirCraftDetails(
        this.itemMasterId,
        this.airCraftDeletedStatus
      )
      .subscribe(
        (data) => {
          const responseData = data;
  ...
  getDeletedATAMappedDataByItemMasterId(event) {
    ...
    this.itemMasterService
      .getDeletedMappedATADetails(this.itemMasterId, this.ataDeletedStatus)
      .subscribe(
        (res) => {
          if (res) {
            this.ataMappedList = res.map((x) => {
              return {
                ...x,
                ataChapterName: x.ataChapterCode + " - " + x.ataChapterName,
                ataSubChapterDescription: x.ataSubChapterId
                  ? x.ataSubChapterCode + " - " + x.ataSubChapterDescription
                  : "",
                createdBy: x.createdBy,
                createdDate: x.createdDate,
  ...
  getPSListOnDeleteStatus(value) {
    this.isSpinnerVisible = true;
    this.itemMasterService.getPurcSaleDetailById(this.itemMasterId).subscribe(
      (res) => {
