...
export class NTAEAlternateComponent implements OnInit {
  ...
  constructor(
    ...
    this.isView = this.authService.checkPermission([
      ModuleConstants.Item_NHATLA + "." + PermissionConstants.View,
    ]);
    this.isNHAAdd = this.authService.checkPermission([
      ModuleConstants.Item_NHATLA + "." + PermissionConstants.Add,
    ]);
    this.isNHAEdit = this.authService.checkPermission([
      ModuleConstants.Item_NHATLA + "." + PermissionConstants.Update,
    ]);
    this.isNHADelete = this.authService.checkPermission([
      ModuleConstants.Item_NHATLA + "." + PermissionConstants.Delete,
    ]);
    this.isNHADownload = this.authService.checkPermission([
      ModuleConstants.Item_NHATLA + "." + PermissionConstants.Download,
    ]);
  }
    ...
    if (this.selectedNTAETabId == 2) {
      this.itemser.getequivalencypartlist(reqData).subscribe((res) => {
        ...
        for (let i = 0; i < this.ntaeData.length; i++) {
          this.pnData.push({
            label: this.ntaeData[i].altPartNo,
            value: this.ntaeData[i].mappingItemMasterId,
          });
          this.ntaeData[i]["MappingItemMasterId"] = this.ntaeData[
            i
          ].mappingItemMasterId;
          this.ntaeData[i]["Description"] = this.ntaeData[i].partDescription;

          this.filterManufacturerData.push({
            label: this.ntaeData[i].manufacturer,
            value: this.ntaeData[i].manufacturerId,
          });
          this.filterDiscriptionData.push({
            label: this.ntaeData[i].altPartDescription,
            value: this.ntaeData[i].altPartDescription,
          });
          this.filterPartItemClassificationData.push({
            label: this.ntaeData[i].itemClassification,
            value: this.ntaeData[i].itemClassificationId,
          });
        }
  ...
  enableSave() {
    if (this.selectedNTAETabId == 2) {
      if (
        this.alternateData.attachmentDetails &&
        this.alternateData.attachmentDetails.length > 0
      ) {
        this.disableSave = false;
        this.updateBtn = false;
...
dismissModel() {
  ...
  saveAlternate(saveNtaeTabForm) {
    ...
    if (this.listOfErrors.length > 0) {
      ...
      if (this.selectedNTAETabId == 2) {
        ...
        this.formDataNtae.append(
          "MappingItemMasterId",
          this.alternateData.MappingItemMasterId
        );
        ...
        this.itemser
          .createNTAEFileUploadForEquivalency(this.formDataNtae)
          .subscribe(
            (datas) => {
        ...
        this.itemser.createnhatlaaltequpart(reqDataJson).subscribe(
          (datas) => {
            if (this.selectedNTAETabId == 1) {
              this.alertService.showMessage(
                "Success",
                "Alternate Information Added Successfully",
                MessageSeverity.success
              );
            }
            if (this.selectedNTAETabId == 3) {
              this.alertService.showMessage(
                "Success",
                "NHA Information Added Successfully",
                MessageSeverity.success
              );
            }
            if (this.selectedNTAETabId == 4) {
              this.alertService.showMessage(
                "Success",
                "TLA Information Added Successfully",
                MessageSeverity.success
              );
            }
  ...
  updateAlternate(saveNtaeTabForm) {
    ...
    if (this.listOfErrors.length > 0) {
      ...
      if (this.selectedNTAETabId == 2) {
        this.formDataNtae.append(
          "ItemMappingId",
          this.alternateData.itemMappingId
        );
        this.formDataNtae.append("ItemMasterId", this.itemMasterId);
        this.formDataNtae.append(
          "MappingItemMasterId",
        ...
        this.formDataNtae.append("IsDeleted", "false");
        this.itemser
          .updateNTAEFileUploadForEquivalency(this.formDataNtae)
          .subscribe(
            (datas) => {
              this.alertService.showMessage(
                "Success",
        ...
        this.itemser.updatenhatlaaltequpart(reqDataJson).subscribe(
          (datas) => {
            this.alertService.showMessage(
              "Success",
  ...
  editNTAETab(selectedTab, rowData) {
    ...
    this.pnData.map((x) => {
      if (x.value != rowData.MappingItemMasterId) {
        pnInfo = {
          value: rowData.MappingItemMasterId,
          label: rowData.altPartNo,
        };
      }
    });
    if (pnInfo) {
        ...
        for (let i = 0; i < this.ntaeData.length; i++) {
          ...
          if (partName) {
            if (
              ...
              this.filterDiscriptionData.push(
                this.ntaeData[i].altPartDescription
              );
            }
          }
        }
    ...
    if (this.itemclaColl) {
      for (let i = 0; i < this.ntaeData.length; i++) {
        if (event == this.ntaeData[i].altPartNo) {
          this.filterItemMaster.MappingItemMasterId = this.ntaeData[
            i
          ].mappingItemMasterId;
          this.filterItemMaster.MappingPNumber = event;
          this.filterItemMaster.ManufacturerId = this.ntaeData[
            i
          ].manufacturerId;
          if (this.selectedNTAETabId == 2) {
            this.filterItemMaster.itemClassificationId = this.ntaeData[
              i
  ...
  restoreNTAERow() {
    ...
    this.itemser
      .restoreNTAERow(itemMappingId, this.userName)
      .subscribe((res) => {
        this.alertService.showMessage(
          "Success",
  ...
  deleteNTAERowRecord() {
    this.itemser
      .deleteNTAERow(this.delNTAERow.itemMappingId, this.userName)
      .subscribe((res) => {
        this.alertService.showMessage(
          "Success",
  ...
  deleteNTAEDocRecord() {
    this.commonService
      .GetAttachmentDeleteById(
        this.delNTAEDoc.attachmentDetailId,
        this.userName
      )
      .subscribe((res) => {
