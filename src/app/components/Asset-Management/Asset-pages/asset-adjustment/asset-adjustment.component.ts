...
export class AssetAdjustmentComponent implements OnInit {
    ...
    getAssetList() {
        this.commonService.smartDropDownList('Asset', 'AssetRecordId', 'AssetId',this.authService.currentUser.masterCompanyId,'','',0).pipe(takeUntil(this.onDestroy$)).subscribe(res => {
    ...
    getAssetAdjustmentDataType() {
		this.commonService.smartDropDownList('AssetInventoryAdjustmentDataType', 'AssetInventoryAdjustmentDataTypeId', 'Description',this.authService.currentUser.masterCompanyId,'','',0).pipe(takeUntil(this.onDestroy$)).subscribe(res => {
