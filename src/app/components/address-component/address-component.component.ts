...
export class AddressComponentComponent implements OnInit {
  ...
  ngOnInit() {
	...
	if (this.id !== 0 && this.id !== undefined) {	
		this.purchaseOrderService.getAllEditID(this.id).subscribe(res => {
			const result = res;
			var shipUsertype = 9; 
			var billUsertype = 9; 
			if(result && result.length > 0) {
				result.forEach(x => {
					...
					if(this.userShipingIdList.length == 0) {			
					...
					this.commonService.autoSuggestionSmartuserDropDownList(shipUsertype, '', true, 20, this.userShipingIdList.join()).subscribe(res => {
						...
						if(this.userBillingIdList.length == 0) {			
...
filterCountries(event) {
  ...
  loadModuleListForVendorComp() {
    const addressType = 'PO';
		this.commonService.getModuleListByUserType(addressType).subscribe(res => {
			this.userTypes = res;
  ...
  onShipToSelected(res?, ModuleId?, UserId?,siteId?,poonly?,Shippingid?) {	
		...
		if(this.shipToContactDataList.length > 0){
	...
	onBillToSelected( res?, ModuleId?, UserId?,siteId?,poonly?) {	
		...
		if(userId > 0) {
	     ...
	     if(this.billToContactDataList.length > 0){
  ...
  getUserShipingList(strText = '', moduleId) {
	...
	getPOAddressById(poId) {
			this.isSpinnerVisible = true;
			this.commonService.getPOAddressById(poId).subscribe(res => {
