...
export class DocumentTypeComponent implements OnInit {
    ...
    constructor(private breadCrumb: SingleScreenBreadcrumbService,
        ...
        public documentTypeService: DocumentTypeService, private dialog: MatDialog, private masterComapnyService: MasterComapnyService, private configurations: ConfigurationService) {
    ...
    ngOnInit(): void {
        this.getList();
        this.breadCrumb.currentUrl = '/singlepages/singlepages/app-document-type';
    ...
    getList() {
        this.documentTypeService.getAllWorkFlows().subscribe(res => {
    ...
    save() {
        ...
        if (!this.isEdit) {
            this.documentTypeService.newAction(data).subscribe(() => {
                this.resetForm();
                this.getList();
                this.alertService.showMessage(
                    'Success',
                    `Added  New Document Type Successfully  `,
                    MessageSeverity.success
                );
            })
        } else {
            this.documentTypeService.updateAction(data).subscribe(() => {
    ...
    deleteConformation(value) {
        if (value === 'Yes') {
            this.documentTypeService.deleteAcion(this.selectedRowforDelete.documentTypeId, this.userName).subscribe(() => {
                this.getList();
                this.alertService.showMessage(
                    'Success',
                    `Deleted Document Type Successfully  `,
                    MessageSeverity.success
                );
            })
        } else {
    ...
    getAuditHistoryById(rowData) {
        this.documentTypeService.historyintegration(rowData.documentTypeId).subscribe(res => {
    ...
    getListByStatus(status) {
        ...
        restorerecord:any={}
        restoreRecord(){  
            this.commonService.updatedeletedrecords('DocumentType',
            'DocumentTypeId',this.restorerecord.documentTypeId, ).subscribe(res => {
                this.currentDeletedstatus=true;
                this.modal.close();
    ...
    sampleExcelDownload() {
        const url = `${this.configurations.baseUrl}/api/FileUpload/downloadsamplefile?moduleName=DocumentType&fileName=DocumentType.xlsx`;
    ...
    customExcelUpload(event) {
        ...
        if (file.length > 0) {
            ...
            this.formData.append('file', file[0])
            this.documentTypeService.IntegrationCustomUpload(this.formData).subscribe(res => {
                event.target.value = '';
