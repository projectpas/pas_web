...
@Component({
  selector: 'app-exchange-sales-order-part-number-filter',
  templateUrl: './exchange-sales-order-part-number-filter.component.html',
  styleUrls: ['./exchange-sales-order-part-number-filter.component.scss']
})
export class ExchangeSalesOrderPartNumberFilterComponent implements OnInit, OnDestroy {
  @Input() customer: any;
    ...
    if (this.query.partSearchParamters.includeMultiplePartNumber) {
      ...
      switch (this.query.partSearchParamters.itemSearchType) {
        case ItemSearchType.StockLine:
          ...
          this.stockLineService.searchstocklinefromExchangeQuotepop(searchQuery)
            .subscribe(result => {
        ...
        default:
          this.isSpinnerVisible = true;
          this.itemMasterService.searchitemmasterfromExchangeQuotepop(searchQuery)
            .subscribe(result => {
              this.isSpinnerVisible = false;
  ...
  onPartNumberSelect(part: any) {
    ...
    this.query.partSearchParamters.customerId = this.exchangeQuote.customerId;
  ...
  bindPartsDroppdown(query) {
    ...
    let partSearchParamters = {
      ...
      "customerId": this.exchangeQuote.customerId,
      "custRestrictDER": this.exchangeQuote.restrictDER,
  ...
  searchMultiPartNumbers(): void {
    let partSearchParamters = {
      ...
      "restrictPMA": this.exchangeQuote.restrictPMA,
      "restrictDER": this.exchangeQuote.restrictDER,
      "customerId": this.exchangeQuote.customerId
    }
