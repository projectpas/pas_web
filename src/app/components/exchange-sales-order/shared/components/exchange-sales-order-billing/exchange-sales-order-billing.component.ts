...
import { InvoiceTypeEnum } from '../../../../sales/order/models/sales-order-invoice-type-enum';
import { NgbModalRef, NgbModal } from '@ng-bootstrap/ng-bootstrap';
declare var $: any;
@Component({
  selector: 'app-exchange-sales-order-billing',
  templateUrl: './exchange-sales-order-billing.component.html',
  styleUrls: ['./exchange-sales-order-billing.component.scss'],
  animations: [fadeInOut]
})
export class ExchangeSalesOrderBillingComponent implements OnInit {
  isViewMode = false;
  @Input() parts: any = [];
  @Input() salesOrderId;
  ...
  billingorInvoiceForm: ExchangeSalesOrderBillingAndInvoicing;
  ...
  onSelectPartNumber(rowData) {
    if (rowData.exchangeSalesOrderPartId != 0 && rowData.exchangeSalesOrderShippingId != 0) {
      this.selectedQtyToBill = rowData.qtyToBill;
      this.partSelected = true;
      this.showBillingForm = true;
      this.isMultipleSelected = false;
      this.salesOrderShippingId = rowData.exchangeSalesOrderShippingId;
      this.getBillingAndInvoicingForSelectedPart(rowData.exchangeSalesOrderPartId, rowData.exchangeSalesOrderShippingId);
    }
  }
  ...
  getBillingAndInvoicingForSelectedPart(partNumber, salesOrderShippingId) {
    ...
    this.exchangeSalesOrderService.getExchangeSalesOrderBillingByShipping(this.salesOrderId, partNumber, salesOrderShippingId).subscribe(result => {
      this.isSpinnerVisible = false;
      if (result) {
        ...
        this.billingorInvoiceForm = new ExchangeSalesOrderBillingAndInvoicing();
      }
  ...
  saveSalesOrderBilling(invoiceStatus: InvoiceTypeEnum) {
    ...
    if (this.isMultipleSelected) {
      this.billingList.filter(a => {
        for (let i = 0; i < a.salesOrderBillingInvoiceChild.length; i++) {
          if (a.salesOrderBillingInvoiceChild[i].selected == true) {
            ...
            p.exchangeSalesOrderShippingId = a.salesOrderBillingInvoiceChild[i].exchangeSalesOrderShippingId;
            p.noOfPieces = a.salesOrderBillingInvoiceChild[i].qtyToBill;
            p.exchangeSalesOrderPartId = a.salesOrderBillingInvoiceChild[i].exchangeSalesOrderPartId;
      ...
      var p = new BillingItems;
      p.exchangeSalesOrderShippingId = this.salesOrderShippingId;
      p.noOfPieces = this.selectedQtyToBill;
      p.exchangeSalesOrderPartId = this.selectedPartNumber;
    ...
    this.billingorInvoiceForm.exchangeSalesOrderId = this.salesOrderId;
    ...
    this.exchangeSalesOrderService.createBilling(this.billingorInvoiceForm).subscribe(result => {
      this.getBillingList();
      this.showBillingForm = false;
      this.salesOrderId = result[0].exchangeSalesOrderId;
  ...
  PrintInvoice() {
    this.isSpinnerVisible = true;
    this.exchangeSalesOrderService.getExchangeSalesOrderBillingInvoicingById(this.salesOrderBillingInvoiceId).subscribe(result => {
      ...
      this.exchangeSalesOrderService.updateExchangeSalesOrderBillingInvoicing(this.salesOrderBillingInvoiceId, billingInvoiceData[0]).subscribe(result => {
        this.getBillingList();
        this.close();
        this.isSpinnerVisible = false;
      });
...
tfoot { display:table-footer-group }
  ...
  PerformBilling() {
    ...
    if (this.isMultipleSelected) {
      this.billingList.filter(a => {
        for (let i = 0; i < a.salesOrderBillingInvoiceChild.length; i++) {
          if (a.salesOrderBillingInvoiceChild[i].selected == true) {
            lastSalesOrderShippingId = a.salesOrderBillingInvoiceChild[i].exchangeSalesOrderShippingId;
            lastSalesOrderPartId = a.salesOrderBillingInvoiceChild[i].exchangeSalesOrderPartId;
          }
        }
      });
    }
  ...
  ViewInvoice(rowData) {
    this.salesOrderId = rowData.exchangeSalesOrderId;
...
export class BillingItems {
  exchangeSalesOrderShippingId: number;
  noOfPieces: number;
