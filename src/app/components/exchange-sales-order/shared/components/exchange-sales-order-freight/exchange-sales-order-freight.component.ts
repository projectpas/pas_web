...
import { ExchangeSalesOrderFreight } from '../../../../../models/exchange/ExchangeSalesOrderFreight';
import { forkJoin } from 'rxjs/observable/forkJoin';
import { NgbModal, NgbModalRef } from '@ng-bootstrap/ng-bootstrap';
import { NgForm } from "@angular/forms";
@Component({
  selector: 'app-exchange-sales-order-freight',
  templateUrl: './exchange-sales-order-freight.component.html',
  styleUrls: ['./exchange-sales-order-freight.component.scss'],
  encapsulation: ViewEncapsulation.None
})
export class ExchangeSalesOrderFreightComponent implements OnInit {
  ...
  createFreightsQuote() {
    ...
    this.isSpinnerVisible = true;
    this.exchangeSalesOrderService.createFreight(sendData).subscribe(result => {
      this.isSpinnerVisible = false;
      this.alertService.showMessage(
        '',
        'Exchange Sales Order Freight saved successfully',
  ...
  openInterShipViaHistory(content, rowData) {
    if (rowData && rowData.exchangeSalesOrderFreightId) {
      this.exchangeSalesOrderService.getExchangeSalesOrderFreightsHistory(rowData.exchangeSalesOrderFreightId).subscribe(
  ...
  restoreRecord() {
    if (this.restorerecord && this.restorerecord.exchangeSalesOrderFreightId > 0) {
      this.commonService.updatedeletedrecords('ExchangeSalesOrderFreight', 'ExchangeSalesOrderFreightId', this.restorerecord.exchangeSalesOrderFreightId,).subscribe(res => {
        this.refreshFreightsOnSaveOrDelete();
        this.modal.close();
  ...
  delete() {
    this.isSpinnerVisible = true;
    this.deleteModal.close();
    if (!this.selectedRowForDelete.exchangeSalesOrderFreightId) {
      ...
      this.alertService.showMessage(
        '',
        'Deleted Exchange Sales Order Freight Successfully',
        MessageSeverity.success
      );
    } else {
      let exchangeSalesOrderFreightId = this.selectedRowForDelete.exchangeSalesOrderFreightId;
      this.exchangeSalesOrderService.deleteexchangeSalesOrderFreightList(exchangeSalesOrderFreightId, this.userName).subscribe(res => {
        this.isSpinnerVisible = false;
        this.alertService.showMessage(
          '',
          'Deleted Exchange Sales Order Freight Successfully',
  ...
  refreshFreightsOnSaveOrDelete(fromDelete = false) {
    this.isSpinnerVisible = true;
    this.exchangeQuoteFreightList = [];
    this.exchangeSalesOrderService.getExchangeSalesOrderFreights(this.exchangeQuoteId, this.deletedStatusInfo).subscribe(res => {
      this.isSpinnerVisible = false;
      ...
      if (this.storedData && this.storedData.length != 0) {
        if (this.deletedStatusInfo == true) {
          ...
          this.storedData.forEach(element => {
            if (element.isDeleted == true && element.exchangeSalesOrderFreightId == 0) {
              res.push(element);
            }
          });
        } else {
          ...
          this.storedData.forEach(element => {
            if (element.isDeleted == false && element.exchangeSalesOrderFreightId == 0) {
              res.push(element)
            }
          });
        }
