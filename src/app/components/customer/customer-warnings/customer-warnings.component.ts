...
export class CustomerWarningsComponent implements OnInit {
  ...
  constructor(
    ...
    this.isWarningAdd = this.authService.checkPermission([
      ModuleConstants.Customers_Warnings + "." + PermissionConstants.Add,
    ]);
    this.isWarningUpdate = this.authService.checkPermission([
      ModuleConstants.Customers_Warnings + "." + PermissionConstants.Update,
    ]);
    this.isWarningDelete = this.authService.checkPermission([
      ModuleConstants.Customers_Warnings + "." + PermissionConstants.Update,
    ]);
    //Next
    this.isDocumentAdd = this.authService.checkPermission([
      ModuleConstants.Customers_Documents + "." + PermissionConstants.Add,
    ]);
    this.isDocumentEdit = this.authService.checkPermission([
      ModuleConstants.Customers_Documents + "." + PermissionConstants.Update,
    ]);
    //Privious
    this.isSalesPersonInfoAdd = this.authService.checkPermission([
      ModuleConstants.Customers_SalesPersonInformation +
        "." +
        PermissionConstants.Add,
    ]);
    this.isSalesPersonInfoEdit = this.authService.checkPermission([
      ModuleConstants.Customers_SalesPersonInformation +
        "." +
    ...
    for (let property in changes) {
      if (property == "selectedCustomerTab") {
        if (
          ...
          changes.selectedCustomerTab.currentValue == "Warnings"
        ) {
  ...
  initWarningData() {
    ...
    if (this.editMode) {
      ...
      this.customerService.getCustomerWarningsById(this.id).subscribe(
        (res) => {
          if (res[0]) {
            ...
            if (warningsData.length > 0) {
              ...
              this.isAllow =
                this.customerWarningData.isAllow == false &&
                this.isCheckAllow == true
                  ? true
                  : this.customerWarningData.isAllow;
              this.isWarning =
                this.customerWarningData.isWarning == false &&
                this.isCheckWarning == true
                  ? true
                  : this.customerWarningData.isWarning;
              this.isRestrict =
                this.customerWarningData.isRestrict == false &&
                this.isCheckRestrict == true
                  ? true
                  : this.customerWarningData.isRestrict;

              this.types.forEach((type) => {
                const index = this.warningMessages.findIndex(
                  (warning) => type.label == warning.sourceModule
                );
                if (index === -1) {
  ...
  async getCustomerWarningsData() {
    this.isSpinnerVisible = true;
    await this.customerService.getCustomerWarningsById(this.id).subscribe(
      (res) => {
  ...
  saveWarnings() {
    ...
    const warningsData = {
      warningsData: [...data],
      customerWarningsId:
        this.customerWarningsId == null || this.customerWarningsId == undefined
          ? 0
          : this.customerWarningsId,
    ...
    this.customerService.saveCustomerwarnings(warningsData).subscribe(
      (res) => {
        this.customerWarningData = res;
