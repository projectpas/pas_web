...
import { formatNumberAsGlobalSettingsModule,getValueFromArrayOfObjectById,getObjectById, getObjectByValue } from "../../generic/autocomplete";
          ...
          (res)=>{
              ...
              res.forEach(element => {
                if(element.label == 'PO Approval') {
                    this.poTaskID = element.value;
                }
             });
             if(this.moduleType  == 'PO') {               
                this.creatingData.approvalTaskId = this.poTaskID;
                this.getPoApprovalList(this.poTaskID);
             }
             this.isSpinnerVisible = false;
          }, err => {
              this.isSpinnerVisible = false;
