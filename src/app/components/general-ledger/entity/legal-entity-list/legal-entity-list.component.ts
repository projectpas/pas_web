...
export class EntityEditComponent implements OnInit, AfterViewInit {
  ...
  permissionAddCheck = [
    ModuleConstants.LegalEntity + "." + PermissionConstants.Add,
    ModuleConstants.LegalEntity_BankingInformation +
      "." +
      PermissionConstants.Add,
    ModuleConstants.LegalEntity_BillingInformation +
      "." +
      PermissionConstants.Add,
  ...
  permissionUpdateCheck = [
    ModuleConstants.LegalEntity + "." + PermissionConstants.Update,
    ModuleConstants.LegalEntity_BankingInformation +
      "." +
  ...
  constructor(
    ...
    this.isDelete = this.authService.checkPermission([
      ModuleConstants.LegalEntity + "." + PermissionConstants.Delete,
    ]);
    this.isDownload = this.authService.checkPermission([
      ModuleConstants.LegalEntityList + "." + PermissionConstants.Download,
    ]);
    this.isContactView = this.authService.checkPermission([
      ModuleConstants.LegalEntity_Contacts + "." + PermissionConstants.View,
    ]);
    this.isBillingView = this.authService.checkPermission([
      ModuleConstants.LegalEntity_BillingInformation +
        "." +
        PermissionConstants.View,
    ]);
    this.isBankingView = this.authService.checkPermission([
      ModuleConstants.LegalEntity_BankingInformation +
        "." +
        PermissionConstants.View,
    ]);
    this.isShippingView = this.authService.checkPermission([
      ModuleConstants.LegalEntity_ShippingInformation +
        "." +
        PermissionConstants.View,
    ]);
    this.isDocumentView = this.authService.checkPermission([
      ModuleConstants.LegalEntity_Documents + "." + PermissionConstants.View,
    ]);
    this.dataSource = new MatTableDataSource();
    if (
      this.entityService.listCollection != null &&
  ...
  dateFilterForTable(date, field) {
    ...
    if (date != "" && moment(date, "MM/DD/YYYY", true).isValid()) {
      ...
      if (
        this.lazyLoadEventDataInput.filters &&
        this.lazyLoadEventDataInput.filters.createdDate
      ) {
        delete this.lazyLoadEventDataInput.filters.createdDate;
      }
      if (
        this.lazyLoadEventDataInput.filters &&
        this.lazyLoadEventDataInput.filters.updatedDate
      ) {
  ...
  openContentEdit(row) {
    ...
    this.route.navigateByUrl(
      `generalledgermodule/generalledgerpage/app-legal-entity-edit/${legalEntityId}`
    );
  }
  ...
  deleteItemAndCloseModel() {
    ...
    this.entityService
      .updateEntitydelete(this.selectedRowforDelete.legalEntityId)
      .subscribe(
        (data) => {
          this.alertService.showMessage(
            "Success",
  ...
  getEntityDataById(id) {
    this.entityService.getEntityDataById(id).subscribe(
      (res) => {
  ...
  openView(row) {
    ...
    this.entityService.getEntityDataById(legalEntityId).subscribe(
      (res) => {
        ...
        if (this.viewDataGeneralInformation.invoiceAddressPosition == 1) {
          ...
          this.viewDataGeneralInformation.invoiceAddressPosition == 2
        ) {
        ...
        if (this.viewDataGeneralInformation.invoiceFaxPhonePosition == 1) {
          ...
          this.viewDataGeneralInformation.invoiceFaxPhonePosition == 2
        ) {
  ...
  getEntityDomesticwireDataById(legalEntityId) {
    this.entityService
      .getEntityDomesticDataById(legalEntityId)
      .subscribe((res) => {
        if (res != null && res && res.length != 0) {
  ...
  getInternationalwireDataById(legalEntityId) {
    this.entityService
      .getEntityInternalDataById(legalEntityId)
      .subscribe((res) => {
        const response = res;
        if (res != null && res && res.length != 0) {
  ...
  exportCSV(dt) {
    ...
    let PagingData = {
      ...
      globalFilter: this.filterText ? this.filterText : "",
    };
